...
public class LinkedList<E extends Comparable<E>> {
	...
	private void removeAll(Node head, E el) {
		if (head == null)
			return;
		if (head == first && head.data.equals(el)) {
			first = first.next;
			head = first;
			n--;
			removeAll(head, el);
		} else if (head.next != null && head.next.data.equals(el)) {
			head.next = head.next.next;
			n--;
			removeAll(head, el);
		} else
			removeAll(head.next, el);
	}
	...
	private void duplicateAll(Node first, E el) {
		if (first == null)
			return;
		if (first.data.equals(el)) {
			Node temp = new Node(el, first.next);
			first.next = temp;
			n++;
			duplicateAll(first.next.next, el);
		} else
			duplicateAll(first.next, el);
	}
